(function () {
  ...
  function appendThinking() {
    ...
    wrap.innerHTML =
      '<div class="chat-msg-label">Josiah</div>' +
  ...
  function sendMessage() {
    ...
    fetch(API_URL, {
      ...
      .catch(function () {
        replaceThinkingWithReply(thinkingEl, "Can't reach the server. 1) Make sure the \"Josiah Server\" window is still open. 2) Open chat from the same server URL (e.g. http://localhost:5000 or your LAN IP:5000). 3) If needed, run START_CHATBOT.bat again.", true, null, null);
      })
      .finally(function () {
        setSendEnabled(true);
      });
