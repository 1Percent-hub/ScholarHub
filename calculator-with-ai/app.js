(function () {
  ...
  (function initAuth() {
    ...
    function setHeaderAvatar(avatarUrl, displayName) {
    ...
    function updateProfileUI(session) {
      ...
      if (session && session.user) {
        ...
        if (auth && auth.isAdmin && auth.isAdmin(session)) {
          if (headerVerifiedBadge) headerVerifiedBadge.classList.add('visible');
        } else {
          if (headerVerifiedBadge) headerVerifiedBadge.classList.remove('visible');
        }
        if (auth) {
          auth.getProfile(session.user.id).then(function (p) {
            const displayName = (p && p.display_name) ? p.display_name : (session.user.email || 'Signed in');
            if (headerProfileName) headerProfileName.textContent = displayName;
            setHeaderAvatar((p && p.avatar_url) || null, displayName);
          }).catch(function () {
            if (headerProfileName) headerProfileName.textContent = session.user.email || 'Signed in';
            setHeaderAvatar(null, session.user.email);
          });
        }
      } else {
  ...
  (function initPasswordToggles() {
    document.addEventListener('click', function (e) {
      var btn = e.target && e.target.closest('.password-toggle-btn');
      if (!btn) return;
  ...
  (function initAccountPanel() {
    ...
    const accountSectionProfile = document.getElementById('account-section-profile');
    const accountSectionSecurity = document.getElementById('account-section-security');
    const accountSectionData = document.getElementById('account-section-data');
    const accountSectionAdmin = document.getElementById('account-section-admin');
    ...
    const accountDataMessage = document.getElementById('account-data-message');
    ...
    function openAccountPanel() {
      ...
      auth.getSession().then(function (r) {
        ...
        accountOverlay.classList.add('account-open');
    ...
    function closeAccountPanel() {
      if (!accountOverlay) return;
      accountOverlay.classList.remove('account-open');
    ...
    function loadAccountInfo() {
      ...
      const accountTabAdmin = document.getElementById('account-tab-admin');
      const accountSectionAdmin = document.getElementById('account-section-admin');
      auth.getAccountInfo().then(function (info) {
        if (!info) return;
        if (accountEmail) accountEmail.value = info.email || '';
        if (accountDisplayName) accountDisplayName.value = info.displayName || '';
        const accountAvatarUrlInput = document.getElementById('account-avatar-url');
        if (accountAvatarUrlInput) accountAvatarUrlInput.value = (info.avatarUrl && info.avatarUrl.indexOf('http') === 0) ? info.avatarUrl : '';
        const accountAvatarEl = document.getElementById('account-profile-avatar');
        if (accountAvatarEl) {
          const name = (info.displayName || info.email || '').trim() || '?';
          const initial = name.charAt(0).toUpperCase();
          if (info.avatarUrl) {
            if (accountAvatarEl.querySelector('img')) {
              accountAvatarEl.querySelector('img').src = info.avatarUrl;
            } else {
              accountAvatarEl.textContent = '';
              const img = document.createElement('img');
              img.src = info.avatarUrl;
              img.alt = '';
              accountAvatarEl.appendChild(img);
            }
          } else {
            accountAvatarEl.textContent = initial;
            const img = accountAvatarEl.querySelector('img');
            if (img) img.remove();
          }
        }
        if (info.isAdmin) {
          if (accountVerifiedBadge) accountVerifiedBadge.classList.add('visible');
          if (accountTabAdmin) {
            accountTabAdmin.hidden = false;
            accountTabAdmin.classList.remove('hidden');
          }
        } else {
          if (accountVerifiedBadge) accountVerifiedBadge.classList.remove('visible');
          if (accountTabAdmin) {
            accountTabAdmin.hidden = true;
            accountTabAdmin.classList.add('hidden');
          }
        }
        if (accountMeta) {
          const parts = [];
          if (info.createdAt) {
            try {
              ...
              parts.push('Account created: ' + d.toLocaleDateString());
            } catch (_) {}
          }
          if (info.lastLoginAt) {
            try {
              const d = new Date(info.lastLoginAt);
              parts.push('Last signed in: ' + d.toLocaleString());
            } catch (_) {}
          }
          accountMeta.textContent = parts.length ? parts.join(' · ') : '';
        }
      });
    }
    ...
    function setMessage(el, text, type) {
      if (!el) return;
      el.textContent = text || '';
      el.className = 'account-form-message' + (type ? ' ' + type : '');
    }

    if (headerAccountBtn) {
      headerAccountBtn.addEventListener('click', openAccountPanel);
    }
    if (accountCloseBtn) {
    ...
    accountTabs.forEach(function (tab) {
      tab.addEventListener('click', function () {
        const t = tab.getAttribute('data-account-tab');
        accountTabs.forEach(function (tb) {
          tb.classList.toggle('account-tab-active', tb === tab);
          tb.setAttribute('aria-selected', tb === tab ? 'true' : 'false');
        });
        [accountSectionProfile, accountSectionSecurity, accountSectionData, accountSectionAdmin].forEach(function (sec) {
          if (!sec) return;
          const match = (t === 'profile' && sec.id === 'account-section-profile') ||
            (t === 'security' && sec.id === 'account-section-security') ||
            (t === 'data' && sec.id === 'account-section-data') ||
            (t === 'admin' && sec.id === 'account-section-admin');
    ...
    if (accountProfileForm) {
      accountProfileForm.addEventListener('submit', async function (e) {
        e.preventDefault();
        ...
        if (!uid) {
          setMessage(accountProfileMessage, 'Not signed in.', 'error');
          return;
        }
        const { error } = await auth.updateProfile(uid, { display_name: accountDisplayName.value.trim() });
        if (error) {
          setMessage(accountProfileMessage, error.message || 'Update failed', 'error');
          return;
        }
        setMessage(accountProfileMessage, 'Profile updated.', 'success');
        if (window.SchoolResourceAuthUpdateProfile) {
          auth.getSession().then(function (r) {
            window.SchoolResourceAuthUpdateProfile(r.data && r.data.session ? r.data.session : null);
          });
        }
      });
    }
    ...
    if (accountPasswordForm) {
      accountPasswordForm.addEventListener('submit', async function (e) {
        e.preventDefault();
        setMessage(accountPasswordMessage, '');
        const newPw = accountNewPassword ? accountNewPassword.value : '';
        const confirmPw = accountConfirmPassword ? accountConfirmPassword.value : '';
        if (newPw !== confirmPw) {
          setMessage(accountPasswordMessage, 'New password and confirmation do not match.', 'error');
          return;
        }
        const auth = window.SchoolResourceAuth;
        if (!auth) return;
        const currentPw = accountCurrentPassword ? accountCurrentPassword.value : '';
        const { error } = await auth.changePassword(currentPw, newPw);
        if (error) {
          setMessage(accountPasswordMessage, error.message || 'Change failed', 'error');
          return;
        }
        setMessage(accountPasswordMessage, 'Password changed.', 'success');
        accountPasswordForm.reset();
      });
    }

    if (accountExportBtn) {
      accountExportBtn.addEventListener('click', async function () {
        setMessage(accountDataMessage, '');
        const ds = window.SchoolResourceData;
        if (!ds || !ds.exportUserData) {
          setMessage(accountDataMessage, 'Export not available.', 'error');
          return;
        }
        const json = await ds.exportUserData();
        if (!json) {
          setMessage(accountDataMessage, 'No data to export.', 'error');
          return;
        }
        try {
          ...
          a.download = 'academic-arsenal-export-' + new Date().toISOString().slice(0, 10) + '.json';
          a.click();
          URL.revokeObjectURL(a.href);
          setMessage(accountDataMessage, 'Data exported. Check your downloads.', 'success');
        } catch (err) {
          setMessage(accountDataMessage, 'Export failed.', 'error');
        }
      });
    }

    if (accountDeleteBtn) {
      accountDeleteBtn.addEventListener('click', async function () {
        if (!window.confirm('Permanently delete your account and all its data? This cannot be undone.')) return;
        setMessage(accountDataMessage, '');
        const auth = window.SchoolResourceAuth;
        if (!auth) return;
        const { error } = await auth.deleteAccount();
        if (error) {
          setMessage(accountDataMessage, error.message || 'Delete failed', 'error');
          return;
        }
        closeAccountPanel();
        if (window.SchoolResourceAuthUpdateProfile) window.SchoolResourceAuthUpdateProfile(null);
        if (window.SchoolResourceAuthShowAuth) window.SchoolResourceAuthShowAuth();
      });
    }

    var accountAvatarBtn = document.getElementById('account-profile-avatar-btn');
    var accountAvatarFile = document.getElementById('account-avatar-file');
    var accountAvatarPopover = document.getElementById('account-avatar-popover');
    var accountAvatarChoose = document.getElementById('account-avatar-choose');
    var accountAvatarRemove = document.getElementById('account-avatar-remove');

    function closeAvatarPopover() {
      if (accountAvatarPopover) accountAvatarPopover.classList.add('hidden');
    }

    if (accountAvatarBtn) {
      accountAvatarBtn.addEventListener('click', function (e) {
        e.stopPropagation();
        if (!accountAvatarPopover) return;
        accountAvatarPopover.classList.toggle('hidden');
      });
    }
    if (accountAvatarChoose) {
      accountAvatarChoose.addEventListener('click', function () {
        if (accountAvatarFile) accountAvatarFile.click();
      });
    }
    if (accountAvatarFile) {
      accountAvatarFile.addEventListener('change', function () {
        var file = accountAvatarFile.files && accountAvatarFile.files[0];
        if (!file) return;
        var auth = window.SchoolResourceAuth;
        if (!auth) return;
        auth.getSession().then(function (r) {
          if (!r.data || !r.data.session) return;
          ...
          reader.onload = function () {
            auth.updateProfile(uid, { avatar_url: reader.result }).then(function () {
              loadAccountInfo();
              if (window.SchoolResourceAuthUpdateProfile) auth.getSession().then(function (s) { window.SchoolResourceAuthUpdateProfile(s.data && s.data.session || null); });
              closeAvatarPopover();
            });
          };
    ...
    if (accountAvatarRemove) {
      accountAvatarRemove.addEventListener('click', function () {
        var auth = window.SchoolResourceAuth;
        if (!auth) return;
        auth.getSession().then(function (r) {
          if (!r.data || !r.data.session) return;
          auth.updateProfile(r.data.session.user.id, { avatar_url: null }).then(function () {
            loadAccountInfo();
            if (window.SchoolResourceAuthUpdateProfile) auth.getSession().then(function (s) { window.SchoolResourceAuthUpdateProfile(s.data && s.data.session || null); });
            closeAvatarPopover();
          });
    ...
    var accountAvatarUrl = document.getElementById('account-avatar-url');
    var accountAvatarFileUpload = document.getElementById('account-avatar-file-upload');
    var accountAvatarApply = document.getElementById('account-avatar-apply');
    if (accountAvatarFileUpload) {
      accountAvatarFileUpload.addEventListener('change', function () {
        var file = accountAvatarFileUpload.files && accountAvatarFileUpload.files[0];
        if (!file) return;
        var auth = window.SchoolResourceAuth;
        if (!auth) return;
        auth.getSession().then(function (r) {
          ...
          reader.readAsDataURL(file);
          accountAvatarFileUpload.value = '';
        });
      });
    }
    if (accountAvatarApply) {
      accountAvatarApply.addEventListener('click', function () {
        var auth = window.SchoolResourceAuth;
        if (!auth) return;
        auth.getSession().then(function (r) {
          if (!r.data || !r.data.session) return;
          var uid = r.data.session.user.id;
          var urlInput = accountAvatarUrl && accountAvatarUrl.value ? accountAvatarUrl.value.trim() : '';
          var fileInput = accountAvatarFileUpload && accountAvatarFileUpload.files && accountAvatarFileUpload.files[0];
          if (fileInput) {
            var reader = new FileReader();
            reader.onload = function () {
              auth.updateProfile(uid, { avatar_url: reader.result }).then(function () {
            ...
            auth.updateProfile(uid, { avatar_url: urlInput }).then(function () {
              loadAccountInfo();
              if (window.SchoolResourceAuthUpdateProfile) auth.getSession().then(function (s) { window.SchoolResourceAuthUpdateProfile(s.data && s.data.session || null); });
            });
          } else {
            auth.updateProfile(uid, { avatar_url: null }).then(function () {
              loadAccountInfo();
              if (window.SchoolResourceAuthUpdateProfile) auth.getSession().then(function (s) { window.SchoolResourceAuthUpdateProfile(s.data && s.data.session || null); });
            });
          }
    ...
    document.addEventListener('click', function () { closeAvatarPopover(); });
    if (accountAvatarPopover) {
      accountAvatarPopover.addEventListener('click', function (e) { e.stopPropagation(); });
    }
  })();
  ...
  function updateDisplay(value) {
    ...
    if (displayEl) {
      displayEl.classList.remove('display-flash');
      displayEl.offsetHeight;
      displayEl.classList.add('display-flash');
      setTimeout(function () { displayEl.classList.remove('display-flash'); }, 350);
    }
  }
  ...
  function applyTheme(themeClass) {
    ...
    if (ds) {
      ds.getSettings().then(function (s) {
        return ds.saveSettings({ theme: next, soundEffects: s.soundEffects, speakOnHover: s.speakOnHover, volume: s.volume });
      }).catch(function () {});
    }
  }
  ...
  function runAppEnterAnimation() {
    if (!appRootEl) return;
    appRootEl.classList.add('app-animate-in');
    setTimeout(function () { appRootEl.classList.remove('app-animate-in'); }, 600);
  }
  ...
  (function initSettingsAndSound() {
    ...
    async function loadSettings() {
      ...
      volumePercent = typeof s.volume === 'number' ? Math.min(100, Math.max(0, s.volume)) : 100;
    }
    ...
    function applyVolume() {
      ...
      const appMusic = document.getElementById('app-music');
      const creditsAudio = document.getElementById('credits-audio');
      if (appMusic) appMusic.volume = v;
      if (creditsAudio) creditsAudio.volume = v;
    }
    ...
    function playClickSound() {
      ...
      try {
        ...
        osc.type = 'sine';
        const gainLevel = 0.12 * (volumePercent / 100);
    ...
    loadSettings().then(function () {
      ...
      const settingVolumeValueEl = document.getElementById('setting-volume-value');
      if (settingSoundEl) settingSoundEl.checked = soundEffectsEnabled;
      if (settingSpeakEl) settingSpeakEl.checked = speakOnHoverEnabled;
      if (settingVolumeEl) {
        settingVolumeEl.value = volumePercent;
        if (settingVolumeValueEl) settingVolumeValueEl.textContent = volumePercent + '%';
      }
      applyVolume();
      const ds = window.SchoolResourceData;
    ...
    if (settingVolumeEl) {
      function updateVolumeFromInput() {
        volumePercent = Math.min(100, Math.max(0, parseInt(settingVolumeEl.value, 10) || 100));
        if (settingVolumeValueEl) settingVolumeValueEl.textContent = volumePercent + '%';
        applyVolume();
        saveSettings();
      }
      settingVolumeEl.addEventListener('input', updateVolumeFromInput);
      settingVolumeEl.addEventListener('change', updateVolumeFromInput);
    }
